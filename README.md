...
一个给 Claude Code 增加“长期记忆感”的轻量插件：少打扰、多观察、会总结、懂提醒。
...
- 🔎 **总结重复踩坑**：把重复失败归纳为 `lessons`，而不是散落的报错碎片
- 🤫 **默认静默运行**：不抢对话、不刷屏，避免影响 Claude Code 正常体验
- 💡 **高价值才提醒**：只在强信号场景下给出温和提示
- 🔐 **本地优先 + 脱敏处理**：减少敏感信息暴露风险
- 🧪 **测试覆盖完整**：核心逻辑、hook 稳定性、提醒策略均有测试保护

> 它不是来“教育你怎么写命令”的，更多是负责在你准备再次踩同一个坑时，小声说一句：
>
> “哥，这个坑你上次踩过。”
...
3. 高频成功命令和典型失败模式，其实都值得沉淀下来，避免反复试错。
4. 但如果提示太频繁、太吵、太自作聪明，体验反而会变差。
...
- 提炼可复用经验，而不是堆原始日志；
- 默认静默运行，而不是疯狂刷存在感；
- 只在高价值场景下提醒，而不是每次都“我有一个想法”。

换句话说，它试图在“完全没记忆”和“话痨型副驾驶”之间，找到一个更克制、更工程化的平衡点。
...
插件会总结成 lesson，而不是只记一堆散乱 stderr。
...
换句话说，它会说“你常这么干”，但不会替你点“允许”。目前它还保留了基本的职业操守。
...
如果看到这些内容，说明插件已经开始“记住你”了。别担心，它目前只记工作，不记黑历史。
...
如果你也受不了每个会话都像在和一位“昨天刚洗掉记忆的天才程序员”重新认识，那这个插件可能正适合你。
...
如果这个项目对你有帮助，未来发到 GitHub 后，欢迎让更多同样被“重复踩坑”和“会话失忆”折磨过的开发者看到它。
